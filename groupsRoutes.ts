...
async function loadGroupDetail(req: Request, res: Response) {
    try {
        ...
        const groupEventsInfo = await client.query(`SELECT records.id as record_id, records.event_id, records.requestor_id, records.receiver_id, records.due, records.amount, events.name,events.date, events.msg FROM records 
        ...
        res.json({ success: true,  groupEventsInfo: groupEventsInfo.rows, groupBalance: groupBalance })
    } catch (e) {
