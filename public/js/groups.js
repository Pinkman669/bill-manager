...
// Create new group
document
	.querySelector('#submit-group-btn')
...
async function loadGroup() {
	...
	if (result.success) {
		...
		result.userGroup.forEach(async (data, i) => {
			...
			groupsBox.innerHTML += `<div class="group-summary-div invisible ${
				groupResult.groupBalance >= 0 ? 'settle' : 'not-settle'
			}">
                                        <div class="group-name">
                                            <div>Group: </div>
                                            <a href="group-detail.html?groupID=${
												data.id
											}">${data.name}</a>
                                        </div>
			...
			const groupSummaryDiv = [
				...document.querySelectorAll('.group-summary-div')
			];
