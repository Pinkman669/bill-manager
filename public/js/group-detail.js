...
window.addEventListener('load', async () => {
	...
	if (groupID) {
		...
		const result = await res.json();
		balanceTypeTag.innerHTML += `${
			result.groupBalance >= 0 ? 'You Lent: ' : 'You Borrowed: '
		}
                                        <span id="user-balance">${
											result.groupBalance >= 0
												? '(+)$'
												: '(-)$'
										}${Math.abs(
			result.groupBalance
		...
		if (result.success) {
			const groupMainDiv = document.querySelector(
				'#group-detail-main-div'
			);
			for (let eventInfo of result.groupEventsInfo) {
				const eventDate = new Date(eventInfo.date).toDateString();
				groupMainDiv.innerHTML += `<div class="group-event ${
					eventInfo.due ? `settle` : `not-settle`
				} invisible">
                                                <div class="event-detail">
                                                    <div class="event-name events"><p>Event Name: </p>
                                                        <a href="/event-detail.html?recordId=${
															eventInfo.record_id
														}">${eventInfo.name}</a>
                                                    </div>
