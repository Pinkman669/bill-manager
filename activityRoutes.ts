...
async function createActivity(req: Request, res: Response) {
	try {
		...
		// Insert events
		await client.query(
			`INSERT INTO events (user_id, msg, name, date, method, amount) VALUES ($1, $2, $3, $4, $5, $6)`,
			[
		...
		const eventID = await client.query(
			`SELECT id FROM events WHERE user_id = $1 AND name = $2 AND date = $3 AND method = $4 AND amount =$5`,
			[
				Number(actInfo.requestorID),
				actInfo.actName,
				actInfo.actDate,
...
async function searchUsers(req: Request, res: Response) {
	try {
		...
		const notFriendList = await client.query(
...
async function addFriend(req: Request, res: Response) {
	try {
		...
		const userIDInput = await client.query(
			`SELECT id FROM users WHERE email = $1`,
			[userNameInput]
		);
		const friendisExisted = await client.query(
			`SELECT * FROM friends WHERE user1_id = $1 AND user2_id = $2 OR user1_id = $2 AND user2_id = $1`,
			[userIDInput.rows[0].id, userID]
		);
